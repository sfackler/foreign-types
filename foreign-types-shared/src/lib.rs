...
/// A trait implemented by types which consumes the wrapper and returnes the raw pointer
pub trait IntoForeignType {
    /// The raw C type.
    type CType;
...
impl<T> IntoForeignType for T where T: ForeignType {
    type CType = T::CType;
